unit plugin.monitoramento.datamodule.console;

interface

uses
  System.SysUtils, System.Classes, System.JSON, System.Generics.Collections, System.Threading,
  uUtils, plugin.controller.schemas, plugin.controller.databases,
  plugin.datamodule, FileSearchUnit, System.StrUtils, System.DateUtils;
...
type
  ...
  private
    ...
    FLoopingEmExecucao: boolean;
    FIntervalLooping: integer;
...
begin
  ...
  Result := Result + ' - Ciclo: '+IfThen(FLoopingEmExecucao, 'Ativo', 'Parado');
end;
...
procedure TdmMonitoramento.LoopingExecucao;
var
  lVerificacao: TDateTime;
begin
  lVerificacao := Now;
  while FLoopingEmExecucao do
  begin
    if IncMilliSecond(lVerificacao, FIntervalLooping) < Now then
      Continue;
...
begin
  FIntervalLooping := AMilliseconds;
end;
